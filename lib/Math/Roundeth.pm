...
sub round_even {
 my @res  = map {
   ...
   if ($fr == 0.5) {
      ...
      $sign * POSIX::floor(abs($_) + $Math::Roundeth::half);
   }
 } @_;
 return (wantarray) ? @res : $res[0];
}
...
sub nearest_ceil {
    my $targ = abs(shift);
    my @res  = map { $targ * POSIX::floor(($_ + $Math::Roundeth::half * $targ) / $targ) } @_;
...
sub nearest_floor {
    my $targ = abs(shift);
    my @res  = map { $targ * POSIX::ceil(($_ - $Math::Roundeth::half * $targ) / $targ) } @_;
...
sub nearest_rand {
 ...
 my @res = map {
   ...
   if ($fr == 0.5 * $targ) {
      ...
      $sign * $targ * int((abs($_) + $Math::Roundeth::half * $targ) / $targ);
   }
 } @_;
 return (wantarray) ? @res : $res[0];
}
...
Math::Roundeth - Perl extension for rounding numbers

=head1 SYNOPSIS

  use Math::Roundeth qw(...those desired... or :all);
...
B<Math::Roundeth> supplies functions that will round numbers in different
...
The variable B<$Math::Roundeth::half> is used by most routines in this
...
Math::Roundeth was written by Geoffrey Rommel E<lt>GROMMEL@cpan.orgE<gt>
in October 2000.
